...
public class PokerHand {
	 ...
	 * of cards.
	 * 
	 * @return true if we have a straight
	 */
	public boolean isStraight() {
		sortNoSuit();
		for (int i = 0; i < cards.length; i++)
			if (i > 0 && cards[i - 1].getRank() != cards[i].getRank() - 1)
				return false;
		return true;
	}
	...
	public boolean isFullHouse() {
		...
		return (cards[0].getRank() == cards[1].getRank() && cards[2].getRank() == cards[3].getRank()
				...
				|| (cards[0].getRank() == cards[1].getRank() && cards[0].getRank() == cards[2].getRank()
	...
	private int findIndexOfMinIgnoreSuit(int start) {
		int indexOfMin = start;
		for (int j = start + 1; j < cards.length; ++j) {
			// look at the two cards, the minimal one we've found,
			// and the one at the current position j
	...
	private void sortNoSuit() {
		for (int i = 0; i < cards.length - 1; ++i) {
			// Step 1: find the "minimal" element to the right of position i
			int indexOfMin = findIndexOfMinIgnoreSuit(i);
